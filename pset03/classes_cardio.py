...
class Playlist:
    ...
    def play_all(self):
        for song in self.songs: #cycles through songs and plays each of them from play method in Song class
            song.play() 
    
    def __str__(self):
        playlist = []
        for song in self.songs[::]:
            playlist.append(f"{song.title} by {song.artist} ({song.duration}s)") #adds songs to playlist
        if self.songs[::] == []:
            return "Playlist is empty."
        return "|". join(playlist) #joings songs together and returns a full playlist
