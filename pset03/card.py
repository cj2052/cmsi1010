...
def deal_one_five_card_hand(cards_per_hand):
    deck = shuffled_deck()
    return set(deck[:cards_per_hand])


def deal(number_of_hands, cards_per_hand): #returns a hand of random cards from a shuffled deck
    hands = []
    [hands.append(deal_one_five_card_hand(cards_per_hand)) for count in range(number_of_hands)]
    return hands 
